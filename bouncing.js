...
var Bouncer = function(elem) {
  ...
  this.$space = $('<span class="space">&nbsp;</span>');
  this.timers = [];
...
Bouncer.prototype.bounce = function() {
  ...
  for (var j = 0; j < this.contentArray.length; j++) {
    ...
    var ballLeft = $word.position().left + wordLength/2;
    var ballTop = $word.position().top;
    ...
    var timer = setTimeout((function(instance, ballProps) {
      return function() {
        ...
        if(ballProps.top > $(window).height()-600){
          $("html, body").animate({ scrollTop: ballProps.top });
          // $("html").animate({ scrollTop: ballProps.top }, 600);
          // $(document).scrollTop(ballProps.top);
        }
      };
